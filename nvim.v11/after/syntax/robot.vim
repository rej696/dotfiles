...
" Matches
"------------------------------------------------------------------------
"syn region keywordSection start=/\*\*\* Keywords \*\*\*/ end=/\(\*\*\* \w\+ \*\*\*\)\|$/
syn match robotGherkin              "\c\s\{2\}\<\(Given\|When\|Then\|And\|But\)\>"
"syn match robotKeywordCall          "\c\(\s\{2\}\|\(Given\|When\|Then\|And\|But\)\s\)\<\(\(\w\+\|\".*\"\)\(\s\{0,1\}\)\)\+\>" contains=robotString,robotGherkin contained containedin=keywordSection
syn match robotKeywordDefinition    "^\c\(\(\(\w\|-\|_\)\+\|\".*\"\)\(\s\{0,1\}\)\)\+" contains=robotString
"syn match robotKeywordCall          "\c\(\w\+\s\)\+"
...
"syn match robotPythonImmediate      "\${{.*}}" contains=robotVariable,robotNumber
"syn region robotVariable            start="\(\$\|@\|&\|%\){" end="}" contained
"syn region robotPythonImmediate     start=+\${{+ excludenl end=+}}+ contains=robotPythonImmediateInner
syn match robotVariable             "\(\$\|@\|&\|%\){.\{-}}" contained
syn match robotVariable2            "\(\$\|@\|&\|%\){[^{].\{-}}"
syn match robotPythonImmediateInner "\${{\zs.*\ze}}" contained contains=robotVariable,robotNumber,robotString
syn match robotPythonImmediate      "\${{.*}}" contains=robotPythonImmediateInner
"syn match robotPythonImmediate      "\${{.\{-}}}" contains=robotPythonImmediateInner
"syn match robotPythonImmediateInner "\${{\<.\{-}\>}}" contained containedin=robotPythonImmediate contains=robotVariable
"syn match robotVariable             "\(\$\|@\|&\|%\){.\{-}}" contained containedin=robotPythonImmediateInner

"syn match robotVariable             "\(\$\|@\|&\|%\){[^{]\{1}.\{-}}" contained
"syn match robotPythonImmediate      "\${{.*}}" contains=robotVariable
...
syn match requestsLibrary        "\c\(\s\{2\}\|^\)\<\(CONNECT\|CONNECT On Session\|Create Client Cert Session\|Create Custom Session\|Create Digest Session\|Create Ntlm Session\|Create Session\|DELETE\|Delete All Sessions\|DELETE On Session\|GET\|Get File For Streaming Upload\|GET On Session\|HEAD\|HEAD On Session\|Last Response\|OPTIONS\|OPTIONS On Session\|PATCH\|PATCH On Session\|POST\|POST On Session\|PUT\|PUT On Session\|Request Should Be Successful\|Session Exists\|Status Should Be\|TRACE\|TRACE On Session\|Update Session\)\>\(\s\{2\}\|$\)"
...
syn region robotString      start=+"+ skip=+\\"+ excludenl end=+"+ contains=robotVariable,robotPythonImmediate contained containedin=robotVariable
syn region robotString2      start=+"+ skip=+\\"+ excludenl end=+"+ contains=robotVariable,robotPythonImmediate
"syn region robotString3     start=+'+ skip=+\\'+ excludenl end=+'+ contains=robotVariable contained containedin=robotVariable
"syn region robotString4      start=+'+ skip=+\\'+ excludenl end=+'+ contains=robotVariable
syn match robotEscapedQuote    /\\"/
...
" Links
"------------------------------------------------------------------------
hi def link robotPythonImmediate    Function
hi def link robotPythonImmediateInner    Special
hi def link robotVariable           Type
hi def link robotVariable2          Type
...
hi def link robotString2            String
"hi def link robotString3            String
"hi def link robotString4            String
...
hi def link robotKeyword            Keyword
hi def link robotKeywordDefinition  Function
"hi def link robotKeywordCall        Function

hi def link builtInLibrary          Function
hi def link collectionsLibrary      Function
hi def link requestsLibrary      Function
...
hi def link stringLibrary           Function
